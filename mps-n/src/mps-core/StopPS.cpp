...
void StopPS::onInitInstance()
{
    ...
    const mps::core::SignalList* list = port->signalList();

    _stopSignal = list->at(0);

    Pt::uint32_t sigIdx = list->getSignalIndex(_stopSignal);
...
void StopPS::onUpdateDataValue(Pt::uint32_t noOfRecords, Pt::uint32_t sourceIdx, const mps::core::Port* port, const Pt::uint8_t* data)
{	
    if( _stoping)
        return;
