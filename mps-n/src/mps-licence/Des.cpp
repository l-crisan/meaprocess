...
const Pt::uint8_t* Des::encrypt(const Pt::uint8_t *Text1, int size)
{
    int i,a1,j,nB,m,iB,k,K,B[8],n,d,round;
    Pt::uint8_t *Text= &text[0];
    memcpy(Text,Text1, size);
    i=size;
    int mc=0;
    a1=i%8;
    if(a1!=0) 
        for(j=0;j<8-a1;j++,i++) Text[i]=' '; Text[i]='\0';

    keygen();

    for(iB=0,nB=0,m=0;m<(size/8);m++) //Repeat for TextLenth/8 times.
    {
...
const Pt::uint8_t * Des::decrypt(const Pt::uint8_t *Text1, int size)
{
