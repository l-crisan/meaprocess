 ...
 */
#ifndef MPAL_VM_MAXINST_H
#define MPAL_VM_MAXINST_H
...
public:
    ...
    virtual int execute(VMContext& context)
    {
        ...
        T* max = (T*) (*(std::size_t*) (context.stackPtr));
            
        for(Pt::uint32_t i = 1; i < count; ++i)
        {
            context.stackPtr -= sizeof(std::size_t);
        ...
        _result->setOperandValue(context.basePtr, *max);
