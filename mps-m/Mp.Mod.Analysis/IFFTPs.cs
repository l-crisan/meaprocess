...
{
    internal class IFFTPs : WorkPS
    {
        public IFFTPs()
        {
            base.Type = "Mp.Analysis.PS.IFFT";
            base.Text = "IFFT";
            base.Group = StringResource.Analysis;
            base.Symbol = Resource.IFFT;
            base.Icon = Resource.IFFTIcon;
        ...
        public override string Description
        {
            get
            {
                return StringResource.IFFTPsDescription;
            }
        }

        protected override void OnDocumentChanged()
        {
            uint ipartSig = (uint)XmlHelper.GetParamNumber(XmlRep, "chn1");

            if (!IsSignalInPortsAvailable(ipartSig, true, null))
            {
                XmlHelper.SetParamNumber(XmlRep, "chn1", "uint32_t", 0);
            }
            else
            {
                if( OutputPorts[0].SignalList != null)
                {
                    if (OutputPorts[0].SignalList.ChildNodes.Count > 0)
                    {
                        XmlElement xmlSignal = Document.GetXmlObjectById(ipartSig);
                        double rate = XmlHelper.GetParamDouble(xmlSignal, "samplerate");
                        XmlElement xmlOutSignal = (XmlElement) OutputPorts[0].SignalList.ChildNodes[0];
                        XmlHelper.SetParamDouble(xmlOutSignal, "samplerate", "double", rate);
                    }
                }                
            }

            uint rpartSig = (uint)XmlHelper.GetParamNumber(XmlRep, "chn2");

            if (!IsSignalInPortsAvailable(rpartSig, true, null))
                XmlHelper.SetParamNumber(XmlRep, "chn2", "uint32_t", 0);
        }
        ...
        protected void OnPropertyDataPort(object sender, EventArgs e)
        {
            ...
            PortDlg dlg = new PortDlg(Document, port.SignalList, XmlRep);
            dlg.ShowDialog();
        ...
        {
            Document.ShowHelp(this.Site, 980);
        }

        protected override void OnProperties(object sender, EventArgs e)
        {
            IFFTPsDlg dlg = new IFFTPsDlg(Document, InputPorts[0].SignalList, XmlRep);
