...
{
    internal class SignalControlPS : WorkPS
    {
        public SignalControlPS()
        {
            base.Type = "Mp.Event.PS.Ctrl";
            base.Text = StringResource.Control;
            base.Group = StringResource.SignalControl;
            base.Symbol = Images.ControlImg;
            base.Icon =  Images.ControlIcon;
            base.IsSingleton = false;
        }

        public override string RuntimeModule
        {
            get { return "mps-event"; }
        }
        ...
        public override string Description
        {
            get
            {
                return StringResource.ControlDescription;
            }
        }
        ...
        protected override void OnDocumentChanged()
        {
            ...
            {
                XmlElement xmlOutSignal =  (XmlElement) outPort.SignalList.ChildNodes[i];

                uint inSigId = (uint) XmlHelper.GetParamNumber(xmlOutSignal,"inSignal");

                if (!IsSignalInPortsAvailable(inSigId, true, null))
                {
                    Document.RemoveXmlObject(XmlHelper.GetObjectID(xmlOutSignal));
                    --i;
                    continue;
                }
                else
                {
                    XmlElement xmlInSignal = Document.GetXmlObjectById(inSigId);
        ...
        {
              Document.ShowHelp(this.Site, 1350);
        }
        ...
        {
            SignalControlDlg dlg = new SignalControlDlg(XmlRep, Document, InputPorts[0].SignalList, OutputPorts[0].SignalList);
