...
{
    public class ClockPS : ProcessStation
    {
        public ClockPS()
        {
            base.Type   = "Mp.Clock.PS.Clock";
            base.Text   = StringResource.Clock;
            base.Group  = StringResource.Time;
            base.Symbol = Images.Clock;
            base.Icon = Images.ClockIcon;
        }
        ...
        public override string Description
        {
            get
            {
                return StringResource.ClockPsDescription;
            }
        }
        ...
        {
            ClockPortDlg Dlg = new ClockPortDlg(Document, port.SignalList, this.XmlRep);
            Dlg.ShowDialog();
        }

        public override void OnHelpRequested()
        {            
            Document.ShowHelp(this.Site, 340);
        }
        ...
        protected override void OnValidate(List<ValidationInfo> valInfoList)
        {
            ...
            {
                string msg = String.Format(StringResource.DataOutPortConError,this._text);
                valInfoList.Add(new ValidationInfo(msg, ValidationInfo.InfoType.Error));
            }

            if (dataOutPort.SignalList.InnerText == "")
            {
                string msg = String.Format(StringResource.DataOutPortSigError,this._text);
                valInfoList.Add(new ValidationInfo(msg, ValidationInfo.InfoType.Error));
            }
        }
    }
