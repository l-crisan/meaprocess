...
{
    public partial class CounterDlg : Form
    {
        ...
        public CounterDlg(XmlElement xmlPS, Document doc, XmlElement xmlInSignalList, XmlElement xmlOutSignalList)
        {
            _xmlPS = xmlPS;
            _doc = doc;
            ...
            typeCol.Items.Add(StringResource.Event);
            typeCol.Items.Add(StringResource.Maxima);
            typeCol.Items.Add(StringResource.Minima);
            typeCol.Items.Add(StringResource.ZeroCrossing);
            typeCol.Items.Add(StringResource.ZeroCrossingRising);
            typeCol.Items.Add(StringResource.ZeroCrossingFalling);

            LoadData();

            FormStateHandler.Restore(this, "Mp.Stat.CounterDlg");
        ...
        private void LoadData()
        {
            ...
            foreach (XmlElement xmlOutSignal in _xmlOutSignalList.ChildNodes)
            {
                ...
                row.Cells[3].Value = XmlHelper.GetParamDouble(xmlOutSignal, "physMin");
                row.Cells[4].Value = XmlHelper.GetParamDouble(xmlOutSignal, "physMax");
                row.Cells[5].Value = XmlHelper.GetParam(xmlOutSignal, "unit");
                row.Cells[6].Value = XmlHelper.GetParam(xmlOutSignal, "comment");
            }
        }
        ...
        private string GetSigType(int type)
        {
            switch (type)
            {
                ...
                case 3:
                    return StringResource.ZeroCrossing;
                case 4:
                    return StringResource.ZeroCrossingRising;
                case 5:
                    return StringResource.ZeroCrossingFalling;
            }
        ...
        private void channels_DragDrop(object sender, DragEventArgs e)
        {
            ...
            row.Cells[1].Value = "";
            row.Cells[2].Value = StringResource.Event;
            row.Cells[3].Value = 0;
        ...
        {
            if( sigType == StringResource.Event)
                return 0;
            ...
            if (sigType == StringResource.ZeroCrossing)
                return 3;

            if (sigType == StringResource.ZeroCrossingRising)
                return 4;

            if (sigType == StringResource.ZeroCrossingFalling)
                return 5;
        ...
        private void OK_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in outputGrid.Rows)
            {
                ...
                if(row.Cells[1].Value != null)
                    outSigName = row.Cells[1].Value.ToString();
            ...
            foreach (DataGridViewRow row in outputGrid.Rows)
            {
                ...
                if (outSignal == null)
                {
                    outSignal = _doc.CreateXmlObject(_xmlOutSignalList, "Mp.Sig", "Mp.Stat.Sig.Count");
                ...
                XmlHelper.SetParamDouble(outSignal, "physMin", "double", Convert.ToDouble(row.Cells[3].Value));
                XmlHelper.SetParamDouble(outSignal, "physMax", "double", Convert.ToDouble(row.Cells[4].Value));

                XmlHelper.SetParam(outSignal, "unit", "string", row.Cells[5].Value.ToString());
                XmlHelper.SetParam(outSignal, "comment", "string", row.Cells[6].Value.ToString());

                XmlHelper.SetParamNumber(outSignal, "valueDataType", "uint8_t", (int)SignalDataType.DINT);
        ...
        private void outputGrid_RowValidating(object sender, DataGridViewCellCancelEventArgs e)
        {
            if (e.RowIndex == -1)
                return;

            DataGridViewRow row = outputGrid.Rows[e.RowIndex];
            int min = Convert.ToInt32(row.Cells[3].Value);
            int max = Convert.ToInt32(row.Cells[4].Value);

            errorProvider.Clear();
            if (min >= max)
            {
                errorProvider.SetError(outputGrid, StringResource.MinMaxErr);
                e.Cancel = true;
            }
        }
        ...
        private void CounterDlg_FormClosing(object sender, FormClosingEventArgs e)
        {
            FormStateHandler.Save(this, "Mp.Stat.CounterDlg");
        }

        private void help_Click(object sender, EventArgs e)
        {
            Document.ShowHelp(this, 1540);
        }
    }
}
