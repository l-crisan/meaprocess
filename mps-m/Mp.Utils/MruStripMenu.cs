...
//					MenuItems[i].Text = FixupEntryname(startNumber, MenuItems[i].Text.Substring(startNumber == 10 ? 5 : 4));
		...
		public void LoadFromRegistry()
		{
			if (registryKeyName != null)
			{
                ...
                string file = path + "\\" + registryKeyName + ".mcfg";
                ...
                try
                {
                    ...
                    foreach(XmlElement xmlFile  in  xmlDoc.DocumentElement.ChildNodes)
                    {
                        if (xmlFile.Name != "file")
                            continue;

                        AddFile(xmlFile.InnerText);
                    }
                }
                catch (Exception ex)
                {
		...
		public void SaveToRegistry()
		{
			if (registryKeyName != null)
			{
				...
				{
                    XmlElement xmlElement = xmlDoc.CreateElement("file");
                    xmlElement.InnerText = ((MruMenuItem)MenuItems[i]).Filename;
